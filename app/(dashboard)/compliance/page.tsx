...
export default async function CompliancePage() {
  ...
  // Spray records double as chemical register entries — derive compliance from real withhold data
  const sprays = orgId ? await getSprayRecords(orgId) : [];
  const today  = new Date();

  // Unique products from spray records — this is the de-facto chemical register
  ...
  for (const s of sprays) {
    ...
    const inWithhold = s.withhold_days > 0 && withholdEnd > today;
    ...
    if (!existing || new Date(s.applied_at) > new Date(existing.lastUsed)) {
      chemicalMap.set(s.product, {
        ...
        status:       inWithhold ? "active" : "active",
      });
    }
  }
  const chemicals = Array.from(chemicalMap.values());

  // Withholding compliance tasks from spray records
  const withholdTasks = sprays
    ...
    .map((s) => {
      ...
      return { id: s.id, title: `Withholding period — ${s.product} on ${s.field_name}`, dueDate: end, priority: "high" as const };
    });
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-6">
        ...
        {/* Chemical register from spray records */}
        <Card padding={false}>
